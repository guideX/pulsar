...
namespace Pulsar.FileSystem.PulsarFileSystem {
    class PulsarFSBlock {
        ...
        public Byte[] Content;
        public PulsarFSBlock(Byte[] Data, Partition p, ulong bn) {
            _BlockNumber = bn;
            _Partition = p;
        ...
        public static PulsarFSBlock Read(Partition p, ulong bn) {
            Byte[] data = p.NewBlockArray(1);
            p.ReadBlock(bn, 1, data);
            return new PulsarFSBlock(data, p, bn);
        }
        public static void Write(Partition p, PulsarFSBlock b) {
        ...
        public static PulsarFSBlock GetFreeBlock(Partition p) {
            for(ulong i = 1; i < p.BlockCount; i++) {
                PulsarFSBlock b = Read(p, i);
                if(!b.Used) {
                    return b;
                }
            }
